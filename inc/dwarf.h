...
int file_name_by_info(const struct Dwarf_Addrs *addrs, Dwarf_Off offset,
                      char *buf, int len, Dwarf_Off *line_off);
int line_for_address(const struct Dwarf_Addrs *addrs, uintptr_t p,
                     Dwarf_Off line_offset, int *store);
...
// Decode an unsigned LEB128 encoded datum. The algorithm is taken from Appendix
// C of the DWARF 4 spec. Return the number of bytes read.
static inline unsigned long dwarf_read_uleb128(const char *addr,
                                               unsigned int *ret) {
  unsigned int result;
  unsigned char byte;
  int shift, count;
