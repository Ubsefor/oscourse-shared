...
//
// +-------9--------+-------9--------+--------9-------+--------9-------+----------12---------+
// |Page Map Level 4|Page Directory  | Page Directory |   Page Table   | Offset
// within Page  | |      Index     |  Pointer Index |      Index     | Index | |
// +----------------+----------------+----------------+--------------------------------------+
// \----PML4(la)----/\--- PDPE(la)---/\--- PDX(la) --/ \--- PTX(la) --/ \----
// PGOFF(la) ----/
//  \------------------------------ VPN(la) -------------------------/
//
// The PML4, PDPE, PDX, PTX, PGOFF, and VPN macros decompose linear addresses as
// shown. To construct a linear address la from PML4(la), PDPE(la), PDX(la),
// PTX(la), and PGOFF(la), use PGADDR(PML4(la), PDPE(la), PDX(la), PTX(la),
// PGOFF(la)).
...
#define PGADDR(m, p, d, t, o)                                                  \
  ((void *)((m) << PML4SHIFT | (p) << PDPESHIFT | (d) << PDXSHIFT |            \
            (t) << PTXSHIFT | (o)))
