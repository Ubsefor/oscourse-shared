...
#define M2DAYS(m, y)                                                    \
  ((const int[]){0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, \
                 365}[m] +                                              \
   (ISLEAP(y) && (m) > 1))
...
timestamp(struct tm *time) {
  return DAY *
             (Y2DAYS(time->tm_year + 1900) - Y2DAYS(1970) +
...
mktime(int time, struct tm *tm) {
  ...
  while (time >= DAY * (1 + Y2DAYS(year + 1) - Y2DAYS(1970)))
    year++;
  tm->tm_year = year - 1900;
  time -= DAY * (Y2DAYS(year) - Y2DAYS(1970));

  int month = time / (DAY * 32);
  while (time >= DAY * (1 + M2DAYS(month + 1, year)))
    month++;
  tm->tm_mon = month;
...
snprint_datetime(char *buf, int size, struct tm *tm) {
  ...
  snprintf(buf, size, "%04d-%02d-%02d %02d:%02d:%02d", tm->tm_year + 1900,
