...
static int dwarf_read_abbrev_entry(const void *entry, unsigned form, void *buf,
  ...
  case DW_FORM_addr:
    ...
    break;
  case DW_FORM_block2: {
    // Read block of 2-byte length followed by 0 to 65535 contiguous information
    // bytes LAB2 code
  ...
  case DW_FORM_indirect: {
    ...
    entry += count;
    int read = dwarf_read_abbrev_entry(entry, form, buf, bufsize, address_size);
    bytes = count + read;
  } break;
  ...
  while ((unsigned char *)entry < addrs->info_end) {
    ...
    do {
      ...
      if (name == DW_AT_low_pc) {
        count = dwarf_read_abbrev_entry(entry, form, &low_pc, sizeof(low_pc),
                                        address_size);
      } else if (name == DW_AT_high_pc) {
        count = dwarf_read_abbrev_entry(entry, form, &high_pc, sizeof(high_pc),
                                        address_size);
  ...
  do {
    ...
    if (name == DW_AT_name) {
      if (form == DW_FORM_strp) {
        ...
        count = dwarf_read_abbrev_entry(entry, form, &offset,
                                        sizeof(unsigned long), address_size);
...
#pragma GCC diagnostic pop
      ...
      count = dwarf_read_abbrev_entry(entry, form, line_off, sizeof(Dwarf_Off),
                                      address_size);
    } else {
      count = dwarf_read_abbrev_entry(entry, form, NULL, 0, address_size);
    }
    ...
    if (tag == DW_TAG_subprogram) {
      ...
      do {
        ...
        if (name == DW_AT_low_pc) {
          ...
          count = dwarf_read_abbrev_entry(entry, form, &high_pc,
      ...
      if (p >= low_pc && p <= high_pc) {
        *offset = low_pc;
        if (name_form == DW_FORM_strp) {
          ...
          count = dwarf_read_abbrev_entry(fn_name_entry, name_form, &str_offset,
                                          sizeof(unsigned long), address_size);
          if (buf && buflen >= sizeof(const char **)) {
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wpointer-to-int-cast"
            put_unaligned((const char *)addrs->str_begin + str_offset,
                          (char **)buf);
#pragma GCC diagnostic pop
          }
        } else {
          count = dwarf_read_abbrev_entry(fn_name_entry, name_form, buf, buflen,
                                          address_size);
        }
        return 0;
      }
...
int address_by_fname(const struct Dwarf_Addrs *addrs, const char *fname,
                     uintptr_t *offset) {
  ...
  while ((const unsigned char *)pubnames_entry < addrs->pubnames_end) {
    ...
    while (pubnames_entry < pubnames_entry_end) {
      ...
      if (!strcmp(fname, pubnames_entry)) {
        ...
        // find low_pc
        if (tag == DW_TAG_subprogram) {
          // At this point entry points to the beginning of function's DIE
          // attributes and abbrev_entry points to abbreviation table entry
          // corresponding to this DIE. Abbreviation table entry consists of
          // pairs of unsigned LEB128 numbers, the first encodes name of
          // attribute and the second encodes its form. Attribute entry ends
          // with a pair where both name and form equal zero.
          // Address of a function is encoded in attribute with name
          // DW_AT_low_pc. To find it, we need to scan both abbreviation table
          // and attribute values. You can read unsigned LEB128 number using
          // dwarf_read_uleb128 function. Attribute value can be obtained using
          // dwarf_read_abbrev_entry function.
          ...
          do {
            ...
            abbrev_entry += count;
            if (name == DW_AT_low_pc) {
              count = dwarf_read_abbrev_entry(entry, form, &low_pc,
...
int naive_address_by_fname(const struct Dwarf_Addrs *addrs, const char *fname,
                           uintptr_t *offset) {
  const int flen = strlen(fname);
  if (flen == 0)
    return 0;
        ...
        do {
          ...
          if (name == DW_AT_low_pc) {
            ...
            if (form == DW_FORM_strp) {
              unsigned long str_offset = 0;
              count =
                  dwarf_read_abbrev_entry(entry, form, &str_offset,
                                          sizeof(unsigned long), address_size);
