...
uint64_t fd2num(struct Fd *fd) { return ((uintptr_t)fd - FDTABLE) / PGSIZE; }
...
int fd_alloc(struct Fd **fd_store) {
  ...
  for (i = 0; i < MAXFD; i++) {
    fd = INDEX2FD(i);
    if ((uvpml4e[VPML4E(fd)] & PTE_P) == 0 || (uvpde[VPDPE(fd)] & PTE_P) == 0 ||
        (uvpd[VPD(fd)] & PTE_P) == 0 || (uvpt[PGNUM(fd)] & PTE_P) == 0) {
      *fd_store = fd;
...
int fd_lookup(int fdnum, struct Fd **fd_store) {
  ...
  fd = INDEX2FD(fdnum);
  if (!(uvpml4e[VPML4E(fd)] & PTE_P) || !(uvpde[VPDPE(fd)] & PTE_P) ||
      !(uvpd[VPD(fd)] & PTE_P) || !(uvpt[PGNUM(fd)] & PTE_P)) {
    if (debug)
...
int dup(int oldfdnum, int newfdnum) {
  ...
  if ((uvpml4e[VPML4E(ova)] & PTE_P) && (uvpde[VPDPE(ova)] & PTE_P) &&
    ...
    if ((r = sys_page_map(0, ova, 0, nva, uvpt[PGNUM(ova)] & PTE_SYSCALL)) < 0)
      goto err;
  if ((r = sys_page_map(0, oldfd, 0, newfd, uvpt[PGNUM(oldfd)] & PTE_SYSCALL)) <
      0)
    goto err;
