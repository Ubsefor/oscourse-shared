...
duppage(envid_t envid, uintptr_t pn) {
  ...
  int res = sys_page_map(id, (void *) (pn * PGSIZE), envid, (void *) (pn * PGSIZE), (ent & PTE_SYSCALL) | PTE_COW );

  if (res < 0) return res;

  return sys_page_map(id, (void *) (pn * PGSIZE), id, (void *) (pn * PGSIZE), (ent & PTE_SYSCALL) | PTE_COW);
}
