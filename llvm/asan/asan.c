...
int __asan_option_detect_stack_use_after_return =
    PLATFORM_ASAN_USE_AFTER_RETURN;

uptr __asan_load_cxx_array_cookie(uptr *UNUSED p) {
  // Do we even support C++?
  asan_internal_unsupported(__func__);
}

void __asan_poison_cxx_array_cookie(uptr UNUSED p) {
  // Do we even support C++?
  asan_internal_unsupported(__func__);
}
...
void __asan_report_load_n(uptr p, unsigned long sz) {
  asan_internal_invalid_access(p, sz, TYPE_LOAD);
}

void __asan_report_store_n(uptr p, unsigned long sz) {
...
#define FAKESTACK_DECLARE(szclass)                                             \
  static uint8_t asan_fakestack_buffer_##szclass                               \
      [PLATFORM_ASAN_FASESTACK_THREAD_MAX *                                    \
           PLATFORM_ASAN_FAKESTACK_CLASS_##szclass##_N *                       \
           FAKESTACK_CLASS_##szclass##_S +                                     \
       1] __attribute__((aligned(SHADOW_ALIGN)));                              \
  static asan_fakestack_config asan_fakestack_config_##szclass                 \
      [PLATFORM_ASAN_FASESTACK_THREAD_MAX *                                    \
           PLATFORM_ASAN_FAKESTACK_CLASS_##szclass##_N +                       \
       1];                                                                     \
  ...
  void __asan_stack_free_##szclass(uptr dst, size_t sz) {                      \
    asan_internal_fakestack_free(FAKESTACK_CLASS_##szclass##_S, dst, sz);      \
  }
...
void __asan_register_globals(uptr g, uptr n) {
...
void __asan_unregister_globals(uptr g, uptr n) {
  if (PLATFORM_ASAN_USE_REPORT_GLOBALS) {
    __asan_global *globals = (__asan_global *)g;
    for (uptr i = 0; i < n; i++) {
      asan_internal_fill_range(globals[i].beg + globals[i].size,
...
void __asan_register_image_globals(uptr UNUSED ptr) {}

void __asan_unregister_image_globals(uptr UNUSED ptr) {}

void __asan_init_v5(void) {}

void __asan_before_dynamic_init(uptr UNUSED arg) {}
...
void __asan_alloca_poison(uptr UNUSED addr, uptr UNUSED size) {}

void __asan_allocas_unpoison(uptr UNUSED top, uptr UNUSED bottom) {}

void __sanitizer_ptr_sub(uptr UNUSED a, uptr UNUSED b) {}

void __sanitizer_ptr_cmp(uptr UNUSED a, uptr UNUSED b) {}

void __asan_poison_stack_memory(uptr UNUSED addr, size_t UNUSED size) {}

void __asan_unpoison_stack_memory(uptr UNUSED addr, size_t UNUSED size) {}

void __sanitizer_annotate_contiguous_container(const void *UNUSED beg,
