...
bool asan_internal_range_poisoned(uptr base, size_t size, uptr *first_invalid,
                                  bool all) {
  ...
  for (size_t i = 0; i < limit; i++, size -= SHADOW_ALIGN) {
    ...
    if (!SHADOW_ADDRESS_VALID(shadow + i)) {
      ...
      if (!failed_once) {
        // ASAN_LOG("Ignoring addr [%p:%p), out of shadow!", (void *)base, (void
        // *)(base+size));
        failed_once = true;
      }
      continue;
    ...
    // All 8 bytes in qword are unpoisoned (i.e. addressable). The shadow value
    // is 0. All 8 bytes in qword are poisoned (i.e. not addressable). The
    // shadow value is negative. First k bytes are unpoisoned, the rest 8-k are
    // poisoned. The shadow value is k.

    // all is used to check if the entire range is poisoned.
    if ((!all && s != 0 &&
         (size > SHADOW_ALIGN || s < size || s > SHADOW_MASK)) ||
...
void asan_internal_check_range(const void *x, size_t size,
...
void asan_internal_alloc_poison(uptr p, size_t osize, size_t nsize,
                                size_t align) {
  // FIXME: No free is available, so we don't care about tracking our
  // allocations.
  // Furthermore, we ignore our left padding being possibly bigger than
  // SHADOW_ALIGN. However, when free exists either free should know both size
  // and alignment (to be able to calculate the substraction offset) or the
  // alignment must never exceed SHADOW_ALIGN.
  ...
  asan_internal_fill_range(p + align + osize, align + nsize - osize,
                           ASAN_HEAP_RZ);
}

uptr asan_internal_fakestack_alloc(asan_fakestack_config *configs,
                                   uint8_t *entries, size_t entryn,
                                   size_t entrysz, size_t realsz) {
  uint32_t thread_id = 0;
  if (!platform_asan_fakestack_enter(&thread_id))
    return 0;

  configs = asan_internal_fakestack_get_t_configs(configs, thread_id, entryn);
  entries = asan_internal_fakestack_get_t_entries(entries, thread_id, entryn,
                                                  entrysz);
  ...
  asan_internal_fill_range(entry + pad + realsz, entrysz - pad - realsz,
                           ASAN_STACK_RIGHT_RZ);
...
void asan_internal_fakestack_free(size_t entrysz, uptr dst, size_t realsz) {
  ...
  bool **used = (bool **)(dst + entrysz - sizeof(uptr) -
                          PLATFORM_ASAN_FAKESTACK_LEFT_RED_SIZE);
  **used = false;
