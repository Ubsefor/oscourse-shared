...
#define ADDRESS_FOR_SHADOW(x)                                                  \
  (((x) - (uptr)asan_internal_shadow_off) << SHADOW_SCALE)
#define SHADOW_FOR_ADDRESS(x)                                                  \
  (uint8_t *)(((x) >> SHADOW_SCALE) + (uptr)asan_internal_shadow_off)
#define SHADOW_ADDRESS_VALID(x)                                                \
  (((uint8_t *)(x) >= asan_internal_shadow_start &&                            \
    (uint8_t *)(x) < asan_internal_shadow_end) ||                              \
   (((uint8_t *)(x) >= asan_internal_shadow_extra_start &&                     \
     (uint8_t *)(x) < asan_internal_shadow_extra_end)))

#ifdef PLATFORM_ASAN_HAVE_PRINTF
int PLATFORM_ASAN_PRINTF(const char *format, ...);
#define ASAN_LOG(fmt, ...)                                                     \
  PLATFORM_ASAN_PRINTF("[ASAN] " fmt "\n", ##__VA_ARGS__)
#else
#define ASAN_LOG(fmt, ...)                                                     \
  do {                                                                         \
  } while (0)
...
#define DUMP_STACK_AT_LEVEL(p, x)                                              \
  do {                                                                         \
    if (((p) != PLATFORM_ASAN_STACK_LAST && __builtin_frame_address(x)) ||     \
        (x) == 0) {                                                            \
...
void NORETURN platform_asan_fatal(const char *msg, uptr p, size_t width,
...
static inline void NORETURN asan_internal_unsupported(const char *msg) {
  platform_asan_fatal(msg, 0, 0, 0);
}

static inline void NORETURN asan_internal_invalid_access(uptr p, size_t width,
                                                         unsigned access_type) {
...
asan_internal_fakestack_get_t_configs(asan_fakestack_config *config,
                                      uint32_t thread_id, size_t entryn) {
  return &config[thread_id * entryn];
}

static inline uint8_t *asan_internal_fakestack_get_t_entries(uint8_t *entries,
                                                             uint32_t thread_id,
                                                             size_t entryn,
                                                             size_t entrysz) {
